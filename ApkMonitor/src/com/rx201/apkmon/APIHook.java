...
public class APIHook extends Application {
	static final boolean DEBUG = true;  /* Shupeng Xu*/
...
//				APIHook.getSystemContext().getPackageManager().resolveService(null, 0);
    ...
    public static synchronized void LogAccess(String PolicyName, String PolicyUID){
    	...
    	Context temp = APIHook.getSystemContext();
    	String appname = getApplicationName(temp);
    	

    	try {
    		
    	    BufferedWriter out = new BufferedWriter(new FileWriter("/data/local/tmp/access.txt", true));
    	    out.write("*****AppStart*****\n");
    	    out.write("Application Name: ");
    	    out.write(appname);
    	    out.write("\n");
    	    out.write("Access Time: Package Name: Issue\n");
